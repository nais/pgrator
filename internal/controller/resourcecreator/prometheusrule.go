...
func CreatePrometheusRuleSpec(postgres *data_nais_io_v1.Postgres, pgClusterName string, pgNamespace string) *monitoring_v1.PrometheusRule {
	...
	rules := containerVolumeAlertRules(pgClusterName, pgNamespace, alertNamespace)
	rules = append(rules,
		monitoring_v1.Rule{
			Alert: "ClusterIsDown",
...
// containerVolumeAlertRules returns the shared container/volume-level alert rules
// used by both Zalando and CNPG PrometheusRules.
func containerVolumeAlertRules(pgClusterName, pgNamespace, alertNamespace string) []monitoring_v1.Rule {
	return []monitoring_v1.Rule{
		{
